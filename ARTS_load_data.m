        ...
        if isfile([root fname_b])
            ...
            for coherence = 1:6
                for condition = 1:4
                    ...
                    % set up RTs_commons to be all NaNs first...
                    % (otherwise, we get a bunch of zeros when we transfer
                    % the total amount of responses to here)
                    RTs_commons{coherence, condition, sub}(:, :, sess) = NaN(50, 2);
                    % then transfer!
                    RTs_commons{coherence, condition, sub}(1:size(transfer_block, 1), 1:2, sess) = transfer_block;
                end
            end
        else
