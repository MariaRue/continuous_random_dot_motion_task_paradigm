function [S] = init_stimulus(S,discrete_trials,tconst, vert_motion)
...
if discrete_trials == 1 || discrete_trials == 2 % If we've selected for discrete trials
    for trial = 1:S.vp.totaltrials % calculate dot positions for each trial
        ...
        [xy, ~, ~, ~, ~, pre_incoh_mo_coh, ~] = ...
        move_dots( S, discrete_trials, trial, S.discrete_stim_duration, ...
        S.ap_radius, S.vp.coherence_sd, S.vp.direction, S.step, S.Nd, ...
        S.pre_incoh_mo, 0, 0, 0, S.coherence_list, 0, 0, 0, ...
        S.vp.passbandfreq, S.vp.stopbandfreq, S.vp.passrip, ...
        S.vp.stopbandatten, tconst.framerate, S.vp.noise_amplitude, ...
        S.mean_duration, S.sd_duration, S.vp.noise_function, ...
        S.vp.stim_function, S.step, ...
        [], [], vert_motion); % last two zeroes may have to be rewritten 
        % S.iti_vec_v and S.cohs_v, respectively, if there is an error
    ...
    for block = 1:numel(S.vp.condition_vec) % loop through all blocks 
        ...
        % duration (coherent and incoherent periods)
        % (all this below is one giant function call)
        [xy, coherence_frame, coherence_frame_v, mean_coherence, ...
            mean_coherence_v, ~, blocks_shuffled] = ...
        move_dots(S, discrete_trials, 0, S.totalframes_per_block, ...
        S.ap_radius, S.vp.coherence_sd, S.vp.direction, S.step, S.Nd, ...
        0, S.mean_coherence{block}, S.blocks_coherence_cells{block}, ...
        S.coherence_frame{block}, 0, S.ITIS_vec{block}, ...
        S.block_coherent_cells{block}, 0, S.vp.passbandfreq, ...
        S.vp.stopbandfreq, S.vp.passrip, S.vp.stopbandatten, ...
        tconst.framerate, S.vp.noise_amplitude, S.mean_duration, ...
        S.sd_duration, S.vp.noise_function, S.vp.stim_function, ...
        S.step, S.iti_vec_v{block}, ...
        S.cohs_v{block}, vert_motion);
        ...
        S.mean_coherence_v{block} = mean_coherence_v;
        S.blocks_shuffled{block} = blocks_shuffled;
        S.coherence_frame{block} = coherence_frame;
        S.coherence_frame_v{block} = coherence_frame_v;
        ...
        % giant function call
        [xy, coherence_frame, ~, mean_coherence, mean_coherence_v] = ...
        move_dots(S, discrete_trials,0,S.totalnoise_frames{block},S.ap_radius,...
        ...
        S.mean_coherence_noise_v{block} = mean_coherence_v;
