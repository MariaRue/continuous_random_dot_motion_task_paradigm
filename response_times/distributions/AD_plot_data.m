function AD_plot_data(RT_distribution, bins)
...
figure_titles = ["Distribution of RTs (response times)", ...
    "Distribution of log RTs (log response times)"];

% plot all conditions in each figure
for type = 1:2
    % create a figure, set title
    figure('Name', figure_titles(type) );
    for condition = 1:4
        subplot(2, 2, condition);

        % plot histogram
        if type == 1
            histogram(RT_distribution{condition}, bins);
        elseif type == 2
            histogram( log(RT_distribution{condition}), bins );
        end
