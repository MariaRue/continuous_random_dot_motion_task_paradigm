function [output_RTs] = analyse_task_RT_PMF(respMat, B, cohs)
...
output_RTs = cell(length(coh_list), 4);
...
for coherence = 1:length(coh_list)
    ...
    for block = 1:4
        ...
        output_RTs{coherence, block}(end + 1:end + size(transfer_block, 1), 1:5) = transfer_block;
        % modify the 2nd column so that it's the decimal logarithm of
        % itself (this is why we transferred the response time twice)
        output_RTs{coherence, block}(1:end, 2) = log(output_RTs{coherence, block}(1:end, 2));
    end
end

% now, modify the 2nd row of each output_RTs{coherence}{block} so that it's
