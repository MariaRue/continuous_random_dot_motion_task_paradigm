function analyse_data_ratios(type, coh_collapsed, side, input_cohs, ids)
...
%            subjects)
%   coh_collapse = whether you wish to collapse data across coherences (so
%   that everything is displayed vs. abs(coherence))
%            0: no
%            1: yes
    ...
    root = ['C:\experiments\Maria_contin_motion\analysis\behav_synth\'];
            for s = 1:12
                ...
                if isfile([root fname_b]) % && isfile([root fname_s])
                    ...
                    % if we want only unique coherences, need collapse
                    % things into only first three rows
                    if cohs_collapsed == 1
                        for n = 1:3
                            for condition = 1:4
                                ...
                                % each with each
                                sums{3+n, 2}{condition}(1,ids(i)) = sums{4-n, 2}{condition}(1,ids(i)) + sums{3+n, 2}{condition}(1,ids(i)); % R_abscoh
                                sums{3+n, 2}{condition}(2,ids(i)) = sums{4-n, 2}{condition}(2,ids(i)) + sums{3+n, 2}{condition}(2,ids(i)); % L_abscoh
                                sums{3+n, 2}{condition}(3,ids(i)) = sums{4-n, 2}{condition}(3,ids(i)) + sums{3+n, 2}{condition}(3,ids(i)); % M_L_abscoh
                                sums{3+n, 2}{condition}(4,ids(i)) = sums{4-n, 2}{condition}(4,ids(i)) + sums{3+n, 2}{condition}(4,ids(i)); % M_R_abscoh
                                sums{3+n, 2}{condition}(5,ids(i)) = sums{4-n, 2}{condition}(5,ids(i)) + sums{3+n, 2}{condition}(5,ids(i)); % T_abscoh
                                sums{3+n, 2}{condition}(6,ids(i)) = (sums{3+n, 2}{condition}(1,ids(i)) + sums{3+n, 2}{condition}(2,ids(i)))/(sums{3+n, 2}{condition}(1,ids(i)) + sums{3+n, 2}{condition}(2,ids(i)) + sums{3+n, 2}{condition}(3,ids(i)) + sums{3+n, 2}{condition}(4,ids(i))); % (R+L)/(R+L+M_R+M_L) i.e. total proportion correct resp. per unique abs coh
                                sums{3+n, 2}{condition}(7,ids(i)) = (sums{3+n, 2}{condition}(3,ids(i)) + sums{3+n, 2}{condition}(4,ids(i)))/(sums{3+n, 2}{condition}(1,ids(i)) + sums{3+n, 2}{condition}(2,ids(i)) + sums{3+n, 2}{condition}(3,ids(i)) + sums{3+n, 2}{condition}(4,ids(i)));; % (M_R+M_L)/(R+L+M_R+M_L) i.e. total proportion misses per unique abs coh
                                sums{3+n, 2}{condition}(8,ids(i)) = sums{4-n, 2}{condition}(8,ids(i)) + sums{3+n, 2}{condition}(8,ids(i)); % this becomes redundant
                                sums{3+n, 2}{condition}(9,ids(i)) = sums{4-n, 2}{condition}(9,ids(i)) + sums{3+n, 2}{condition}(9,ids(i)); % so does this
                            end
                        end
                    end
...
for i = 1:4
    ...
    ratios_means_cohs_collapsed{i} = NaN(3, 4);
    ...
    case 2
        ...
        for i = 1:4 % FOR each block
            ...
            if cohs_collapsed == 1
                for n = 1:3 % FOR each coherence
                    ...
                    indices = find(~isnan(sums{n, 2}{i}(7, :)));
                    % first, create a matrix which contains all ratios per coherence
                    % per condition across all subjects
                    ratios_together{i, 2}(n, 1:length(indices)) = ...
                        sums{n, 2}{i}(7, indices); % 
                    % the sum of ratios for each coherence for each condition across
                    % all subjects (numerator of the mean formula)
                    ratios_means{i}(n, 2) = nansum(sums{n, 2}{i}(7,:));
                    % and the mean ratio for each coh is...
                    % (divide by participant number b/c there is only one ratio per
                    % coherence per condition for each participant, so the amount of 
                    % ratios we add together to the numerator of our mean formula is 
                    % equal to the amount of participants, which is what we put in the
                    % denominator)
                    ratios_means{i}(n, 4) = ratios_means{i}(n, 2)/part_number;
                end
            else
        ...
        for i = 1:4
            ...
            switch side
                case 1 % rightward resp data
                    % calculate interquartile ranges (IQR) so we can plot it with plot()
                    if cohs_collapsed == 1% do we want to collapse across cohs?
                        ...
                        for n = 1:3
                            ...
                            iqr(n, 1) = quantile(ratios_together{i, 1}(4-n, :) + ratios_together{i, 1}(3+n, :), 0.25); % sum up all unique coherences regardless of sign (branching out from the middle)
                            iqr(n, 2) = quantile(ratios_together{i, 1}(4-n, :) + ratios_together{i, 1}(3+n, :), 0.75);
                        end
                        
                        % fill background (IQRs)
                        fill([unique(abs(coherences)) fliplr(unique(abs(coherences)))], ...
                            [iqr(:, 1).' fliplr(iqr(:, 2).')], ...
                            plotColour('LightRed'), 'LineStyle', 'none');
                        hold on;
                        % plot mean
                        plot(unique(abs(coherences))), ratios_means_cohs_collapsed{i}(:, 2), '-*', ...
                ...
                case 2 % leftward response data
                    ...
                    if cohs_collapsed == 1% do we want to collapse across cohs?
                        ...
                        for n = 1:3
                            % calculate 25%-75% IQRs for each unique
                            % coherence (i.e. unique absolute value)
                            iqr(n, 1) = quantile(ratios_together{i, 2}(4-n, :) + ratios_together{i, 2}(3+n, :), 0.25); % sum up all unique coherences regardless of sign (branching out from the middle)
                            iqr(n, 2) = quantile(ratios_together{i, 2}(4-n, :) + ratios_together{i, 2}(3+n, :), 0.75);
                        end
                        ...
                        hold on;
                        % plot mean
                        plot(unique(abs(coherences)), ratios_means{i}(:, 4), '-*', ...
                             'Color', plotColour('Red'));
