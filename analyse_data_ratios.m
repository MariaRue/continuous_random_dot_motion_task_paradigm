...
for i = 1:length(ids)
    ...
    % variables for analysing data
    RTs_commons = cell(6, 4);
    for i = 1:24
        RTs_commons{i} = cell(6, 1);
    end
    ...
    elseif type == 3 && exist('input_data_ordered','var') == 1
        ...
        for coherence = 1:length(coherences)
            for condition = 1:4
                ...
                if ~isempty( input_data_ordered{coherence, condition}(:, 1:2) )
                    RTs_means{coherence, condition}(end + 1, 1:2) = mean( input_data_ordered{coherence, condition}(:, 1:2) );
                else
                    RTs_means{coherence, condition}(end + 1, 1:2) = [0, 0];
                end
            end
        end
...
switch type
    ...
    case 3
        ...
        for coherence = 1:length(coherences)
            for condition = 1:4
                % mean of RTs
                mean_RTs{condition}(coherence, 1) = mean( RTs_means{coherence, condition}(:, 1) );
                % mean of log(RTs)
                mean_RTs{condition}(coherence, 2) = mean( RTs_means{coherence, condition}(:, 2) );
            end
        end
