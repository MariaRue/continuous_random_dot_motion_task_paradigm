function analyse_data_ratios(type, side, int_control, input_cohs, ids)
...
%            3: response times (per coherence, per condition, per subject,
%            per session (mean))
%            4: false alarm rate (proportion early responses vs. condition)
...
%   to LONG integration trial periods after 3 seconds as misses)
%            0: no
%            1: yes
...
RTs_commons_means = cell(6, 4);
% other stuff for RT analysis
RTs_commons = cell(6, 4);
for i = 1:24
    RTs_commons{i} = cell(100, 1);
end
        ...
        if isfile([root fname_b]) % && isfile([root fname_s])
            ...
            if type == 3 % if measuring reaction times...
                ...
                if int_control == 0 % not controlling for integration length...
                    [input_data, FA] = analyse_task_data_PMF(respMat, B, input_cohs, 0);
                else
                    [input_data, FA] = analyse_task_data_PMF(respMat, B, input_cohs, 1);
                end
            end
            ...
            elseif type == 3
            %% Analysing RTs, collate data from each session
                for coherence = 1:6
                    for condition = 1:4
                        ...
                        RTs_commons{coherence, condition}{ids(i)}(end + 1:end + size(transfer_block, 1), 1:2) = transfer_block;
                    end
                end
            end
        else
...
%% If analysing RTs, CREATE COMMONS
% First, delete empty cells in RTs_commons
for coherence = 1:length(coherences)
    for condition = 1:4
        RTs_commons{coherence, condition} = RTs_commons{coherence, condition}(~cellfun(@isempty, RTs_commons{coherence, condition}));
        % but if we've deleted (a) subject(s) who genuinely did not have
        % any responses, top up the matrix
%         if length(RTs_commons{coherence, condition}) < part_number
%             for i = 1:part_number-length(RTs_commons{coherence, condition})
%                 %% FIX THIS IT CAN'T BE ZERO, THAT COUNTS AS AN INSTANTANEOUS RESPONSE TIME
%                 RTs_commons{coherence, condition}{end+1} = [0 0];
%             end
%         end
    end
end

% Now finds RTs_commons_means.
...
for coherence = 1:length(coherences)
    for condition = 1:4
       ...
       % if ~isempty( RTs_commons{coherence, condition}(:, 1:2) )
            % this input is just going to be one row, as it is a
            % *mean* from one subject, but all of that subject's
            % sessions' worth of response times
            for subject = 1:size(RTs_commons{coherence, condition}, 1)
                RTs_commons_means{coherence, condition}(end+1, 1:2) = mean( RTs_commons{coherence, condition}{subject}(:, 1:2) );
            end
       % else
        %    RTs_commons_means{coherence, condition}(end + 1, 1:2) = [0, 0];
       % end
    end
end
...
switch type
    ...
    case 3
        % find mean of RTs for each coherence. In other words, now we are
        % findng the mean per sesssion, mean per subject *from* just the
        % mean per session, per subject data points we had before for each
        % subject
        ...
        for coherence = 1:length(coherences)
            for condition = 1:4
                % mean of RTs
                mean_RTs{condition}(coherence, 1) = mean( RTs_commons_means{coherence, condition}(:, 1) );
                % mean of log(RTs)
                mean_RTs{condition}(coherence, 2) = mean( RTs_commons_means{coherence, condition}(:, 2) );
            end
        end
            ...
            switch side
                case 1 % rightward resp data
                    ...
                    for coherence = 1:length(coherences)
                        ...
                        % coherence
                        error_bars(coherence) = std( RTs_commons_means{coherence, condition}(:, 2) )/sqrt( length(RTs_commons_means{coherence, condition}(:, 1) ));
                    end
                    % fill background (IQRs)
%                      fill([coherences.' fliplr(coherences).'], ...
...
%                          plotColour('LightBlue'), 'LineStyle', 'none');
                    ...
                    % plot mean
                    hold on;
                    plot(coherences, mean_RTs{condition}(:, 2), '-*', ...
                         'Color', plotColour('Red'));
